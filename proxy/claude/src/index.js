...
const APP_VERSION = "103-2026-09-04";
...
// Клод неоднократно путал два независимых механизма: входные файлы,
// приложенные технологом (передаются текстом/документом прямо в этом
// сообщении), и файловую систему code_execution ($INPUT_DIR — только
// для файлов, которые Клод сам создаёт кодом). Проверка $INPUT_DIR на
// предмет входных вложений всегда даёт "файла нет", даже когда он
// успешно доставлен, и Клод на этом основании ошибочно объявлял сбой
// доставки (задача 397958, несколько раундов переписки впустую).
const FILE_DELIVERY_PROTOCOL = `
КАК ТЕБЕ ПЕРЕДАЮТСЯ ВХОДНЫЕ ФАЙЛЫ:
Файлы, приложенные технологом к этому диалогу (чертежи, референсы, вспомогательные .md/.js/.json и т.п.), передаются тебе напрямую как текст или документ прямо в этом сообщении — каждый такой файл предваряется меткой вида Файл "<имя файла>": и дальше идёт его содержимое.
Эти файлы НЕ загружаются в файловую систему code_execution и никогда не появятся в $INPUT_DIR или где-либо ещё в песочнице — это два независимых, не связанных друг с другом механизма.
Поэтому НЕ проверяй наличие входных приложенных файлов через bash/code_execution (ls, find по $INPUT_DIR и т.п.) — такая проверка ничего не докажет: файла там не будет, даже если он успешно доставлен. Единственный способ убедиться, что файл дошёл, — искать в тексте текущего диалога метку Файл "<имя>":.
Файловая система code_execution ($INPUT_DIR и т.п.) относится только к файлам, которые ты сам создаёшь в процессе выполнения кода (например, итоговый .docx/.pdf) — не к входным вложениям технолога.
`;
...
function buildWhitespaceFlexibleRegExp(
  ...
  // Документы иногда выделяют часть текста правила **жирным** прямо
  // внутри предложения (не только префиксом списка), например
  // "**Материал и теплостойкость.** Для ПП..." — из-за этого просто
  // пробельного разделителя между словами недостаточно: "**" рвёт
  // соседство слов без пробела вокруг. Допускаем *С_ и пробелы вперемешку
  // между словами, плюс опциональные * с краёв всего совпадения.
  const source =
    "[*_]*" +
    String(value)
      .trim()
...
function buildMarkdownListAwareRegExp(
  ...
  // Тот же допуск на встроенное **выделение** внутри строки, что и в
  // buildWhitespaceFlexibleRegExp — см. комментарий там.
  const source = lines
    .map((line) =>
      optionalListPrefix +
      "[*_]*" +
...
function updateMasterInstructionMetadata(
  ...
  const versionPatterns = [
    ...
    // Некоторые master-документы (например, Промт-мастер_Feasibility_
    // Seppra.md) пишут версию без двоеточия — "Версия 1.0.". Пробуем
    // последним, чтобы не перебивать более специфичные форматы выше.
    /(Версия\s+)[0-9]+(?:\.[0-9]+)+/i
  ];
...
function extractMasterInstructionVersion(
  ...
  const patterns = [
    ...
    // Некоторые master-документы пишут версию без двоеточия —
    // "Версия 1.0." (см. симметричный список в
    // updateMasterInstructionMetadata). Последним по приоритету.
    /Версия\s+([0-9]+(?:\.[0-9]+)+)/i
  ];
  ...
  for (const company of companies) {
    try {
      ...
      if (inn) {
        let dup = null;
        try {
          dup = await findPlanfixDuplicateByInn(planfixToken, inn);
        } catch (error) {
          console.error("[lead-search] pre-upload dedup check failed:", error);
        }
        if (dup) {
          results.push({
            name: company.name,
